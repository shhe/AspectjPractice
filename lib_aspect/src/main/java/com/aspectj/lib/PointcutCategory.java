...
/*    @Pointcut("cflow(call(* sub(..))) && target(operations) && if(\"FundamentalOperationsImpl\".equals(operations.getName)) && args(param, ..)")
    public void ifTargetCflow(FundamentalOperations operations, int param) {

    }

    @After(value = "ifTargetCflow(operations, param)")
    public void ifTargetCflowbelowMethod(JoinPoint joinPoint, FundamentalOperations operations, int param) {
        Log.i(TAG, "... targetCflowbelowMethod ... "+joinPoint.getSignature().getName()+" "+joinPoint.getSourceLocation());
    }*/
}
